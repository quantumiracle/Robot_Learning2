...
class ReacherEnv(object):
    ...
    def __init__(self, headless):
        ...
        self.end_point = Dummy('Sawyer_target')
    ...
    def reset(self):
        ...
        pos = list(np.random.uniform(POS_MIN, POS_MAX, 3))
    ...
    def step(self, action):
        pos = list(np.random.uniform(POS_MIN, POS_MAX, 3))
        self.end_point.set_position(pos)
...
if __name__ == '__main__':
    ...
    for step in range(1000):
        print(step)
        action=np.random.uniform(0,1, 7)  # action dim is 7
        try:
            env.step(action)
